/**
 * This is distribute from
  https://github.com/AviatrixSystems/terraform-aviatrix-azure-controller/tree/master/modules/aviatrix_controller_build 
 * # Aviatrix Controller Build on Azure
 *
...
resource "azurerm_network_security_group" "aviatrix_controller_nsg" {
  ...
  security_rule {
    ...
    description                = "all-for-vm-management"
  }
}
// 4-1. Association Security Group with Subnet
# resource "azurerm_subnet_network_security_group_association" "asso_nsg2subnet" {
...
resource "azurerm_linux_virtual_machine" "aviatrix_controller_vm" {
  ...
  name                            = "${var.controller_name}-controller"
